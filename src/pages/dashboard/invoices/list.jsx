...
const useInvoicesStore = (searchState) => {
  ...
  const handleInvoicesGet = useCallback(async () => {
    try {
      ...
      const response2 = await invoicesApi.getInvoices();

      if (isMounted()) {
        setState({
          invoices: response.data,
          invoicesCount: response.count,
          paidInvoices: response2.data.filter((p)=>p.status==="paid"),
          unpaidInvoices: response2.data.filter((p)=>p.status==="pending"),
        });
      }
    } catch (err) {
      console.error(err);
        ...
        >
          ...
          <InvoiceListContainer open={openSidebar}>
            <Stack spacing={4}>
              ...
              >
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={()=> {
                      router.push(paths.dashboard.invoices.create);
                    }}
                  >
                    New
                  </Button>
              ...
              <InvoiceListSummary paidInvoices={invoicesStore.paidInvoices} unpaidInvoices={invoicesStore.unpaidInvoices} />
              <InvoiceListTable
